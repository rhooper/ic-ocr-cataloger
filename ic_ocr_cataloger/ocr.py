...
class OCR:
    ...
    def run(self):
        self.start_ocr_workers(self.app.config, self.ocr_req_queue, self.ocr_res_queue)

    def start_ocr_workers(self, config, ocr_req_queue, ocr_res_queue):
        self.ocr_pool = [
    ...
    async def ocr_task(self):
        await self.app.cam.frame_ready.wait()
        while not asyncio.current_task().cancelled():
            try:
                await self.ocr_single_frame(self.app.cam.cur_cv2_image)
    ...
    async def ocr_frame(
        ...
        self.ocr_req_queue.put(self.app.cam.cur_cv2_image)
                ...
                bboxes.append(
                    BoundingBox(
                        ...
                        ocrmac.convert_coordinates_pil(
                            position,
                            self.app.cam.frame_width,
                            self.app.cam.frame_height,
                        ),
